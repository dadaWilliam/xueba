...
class VideoDetail extends StatefulWidget {
  ...
  const VideoDetail({Key? key, this.pageId, this.id}) : super(key: key);
...
class _VideoDetailState extends State<VideoDetail> {
  ...
  _updateLike() async {
    ...
    if (response.statusCode == 200) {
      if (is_liked == false) {
        if (ID1 != -1) {
          videos_index_show[ID1].liked!.add('new one');
        } else {
          videos_latest[ID2].liked!.add('new one');
        }
      } else {
        if (ID1 != -1) {
          videos_index_show[ID1].liked!.removeLast();
        } else {
          videos_latest[ID2].liked!.removeLast();
        }
      }
  ...
  _updateCollect() async {
    ...
    if (response.statusCode == 200) {
      if (is_collected == false) {
        if (ID1 != -1) {
          videos_index_show[ID1].collected!.add('new one');
        } else {
          videos_latest[ID2].collected!.add('new one');
        }
      } else {
        if (ID1 != -1) {
          videos_index_show[ID1].collected!.removeLast();
        } else {
          videos_latest[ID2].collected!.removeLast();
        }
      }
      if (mounted) {
        var snackBar = SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.pageId != null) {
      video = videos_index_show[widget.pageId!];
      ID1 = widget.pageId!;
      //print("page id" + widget.pageId.toString());
      //print(" name is " + video.title!);
      times++;
    } else if (widget.id != null) {
      video = videos_latest[widget.id!];
      ID2 = widget.id!;
                                ...
                                AwesomeDialog(
                                  ...
                                  btnCancelOnPress: () {
                                    _launchUrl(Uri.parse(
                                        '${AppConstants.URL}${AppConstants.VIDEO}/$video_id/'));
                                  },
                                ).show();
