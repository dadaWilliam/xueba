...
class PopularVideoDetail extends StatefulWidget {
  final int? pageId;
  final int? id;

  const PopularVideoDetail({Key? key, this.pageId, this.id}) : super(key: key);

  @override
  State<PopularVideoDetail> createState() => _PopularVideoDetailState();
}
...
class _PopularVideoDetailState extends State<PopularVideoDetail> {
  ...
  _getVideo() async {
    ...
    videos_popular = Get.find<VideoController>().popularVideoList;
  ...
  _updateLike() async {
    ...
    if (response.statusCode == 200) {
      if (is_liked == false) {
        videos_popular[widget.id].liked!.add('new one');
      } else {
        videos_popular[widget.id].liked!.removeLast();
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.id != null) {
      video = videos_popular[widget.id!];
