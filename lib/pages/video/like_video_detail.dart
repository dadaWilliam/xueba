...
class LikeVideoDetail extends StatefulWidget {
  final int? pageId;
  final int? id;

  const LikeVideoDetail({Key? key, this.pageId, this.id}) : super(key: key);

  @override
  State<LikeVideoDetail> createState() => _LikeVideoDetailState();
}

class _LikeVideoDetailState extends State<LikeVideoDetail> {
  ...
  _getVideo() async {
    ...
    videos_likes = Get.find<UserController>().likedList;
  ...
  _updateLike() async {
    ...
    if (response.statusCode == 200) {
      if (is_liked == false) {
        videos_likes[widget.id].video[0].liked!.add('new one');
      } else {
        videos_likes[widget.id].video[0].liked!.removeLast();
      }
  ...
  _updateCollect() async {
    ...
    if (response.statusCode == 200) {
      if (is_liked == false) {
        videos_likes[widget.id].video[0].collected!.add('new one');
      } else {
        videos_likes[widget.id].video[0].collected!.removeLast();
      }
      if (mounted) {
        var snackBar = SnackBar(
