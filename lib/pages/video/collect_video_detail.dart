...
class CollectVideoDetail extends StatefulWidget {
  final int? pageId;
  final int? id;

  const CollectVideoDetail({Key? key, this.pageId, this.id}) : super(key: key);

  @override
  State<CollectVideoDetail> createState() => _CollectVideoDetailState();
}

class _CollectVideoDetailState extends State<CollectVideoDetail> {
  ...
  _getVideo() async {
    ...
    videos_collects = Get.find<UserController>().collectedList;
  ...
  _updateLike() async {
    ...
    if (response.statusCode == 200) {
      if (is_liked == false) {
        videos_collects[widget.id].video[0].liked!.add('new one');
      } else {
        videos_collects[widget.id].video[0].liked!.removeLast();
      }
  ...
  _updateCollect() async {
    ...
    if (response.statusCode == 200) {
      if (is_liked == false) {
        videos_collects[widget.id].video[0].collected!.add('new one');
      } else {
        videos_collects[widget.id].video[0].collected!.removeLast();
      }
      if (mounted) {
        var snackBar = SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.id != null) {
      video = videos_collects[widget.id!];
