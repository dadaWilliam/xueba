...
class NotificationVideoDetail extends StatefulWidget {
  final int? pageId;
  final int? id;

  const NotificationVideoDetail({Key? key, this.pageId, this.id})
      : super(key: key);

  @override
  State<NotificationVideoDetail> createState() =>
      _NotificationVideoDetailState();
}

class _NotificationVideoDetailState extends State<NotificationVideoDetail> {
  ...
  _getVideo() async {
    ...
    videos_notifications = Get.find<UserController>().notificationList;
  ...
  _updateLike() async {
    ...
    if (response.statusCode == 200) {
      if (is_liked == false) {
        videos_notifications[widget.id].video[0].liked!.add('new one');
      } else {
        videos_notifications[widget.id].video[0].liked!.removeLast();
      }
  ...
  _updateCollect() async {
    ...
    if (response.statusCode == 200) {
      if (is_liked == false) {
        videos_notifications[widget.id].video[0].collected!.add('new one');
      } else {
        videos_notifications[widget.id].video[0].collected!.removeLast();
      }
      if (mounted) {
        var snackBar = SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.id != null) {
      video = videos_notifications[widget.id!].video[0];
